...
This directory is a static design sandbox for trying out visual styles. It is **not** the source of truth for the generated pages — the real templates live in `web/*.tpl` and are rendered into `docs/` by the Go program. `dashboard-mockup/` is edited by hand, isn't regenerated by any workflow, and can drift out of sync with the actual dashboard.
